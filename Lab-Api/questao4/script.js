...
document.querySelector("#addBook").addEventListener('click', function(){
    ...
    if(bookTitle !== '' && bookAuthor !== ''){
        if(searchBook === undefined){
            ...
            p.innerHTML = `O livro ${bookTitle} foi adicionado ao banco de livros.`;
            loadAllBooks(books);
            loadAvailableBooks(books);
        }
        else{
            p.innerHTML = `O livro ${bookTitle} já está presente no banco de livros`;
        }
    }
    else{
        p.innerText = `Digite todas as informações corretamente`;
    }
    bookTitleEl.value = "";
    bookAuthorEl.value = "";
});
...
document.querySelector("#borrowBook").addEventListener('click', () =>{
    ...
    if(bookTitleEl.value !== ""){
        ...
        p.innerHTML = 'Digite o título do livro';
    }
    bookTitleEl.value = '';
});
...
document.querySelector("#returnBook").addEventListener('click', () =>{
    ...
    if(book !== ""){
        ...
        p.innerHTML = 'Digite o título do livro que você quer devolver';
    }
    bookTitleEl.value = '';
});
